                ...
                Some(c) => {
                    match c {
                        ...
                        'e' => {
                            ...
                            return Ok(BenCodeExpr::Int(result.as_str().parse::<i64>().unwrap()));
                        }
                        _ => {
                            return Err(format!(
        ...
        loop {
            match self.chars.peek() {
                Some(c) => match c {
                    ...
                    // The ? in the following code is syntactic sugar for the following code
                    // match expr {
                    //     Some(e) => e,
                    //     Err(e) => return expr
                    // }
                    // It must be in a function that returns a Result
